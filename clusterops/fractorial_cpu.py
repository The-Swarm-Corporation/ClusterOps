...
def set_cpu_affinity(cpu_id):
    try:
        if platform.system() == "Windows":
            import win32process
            win32process.SetProcessAffinityMask(os.getpid(), 1 << cpu_id)
        elif platform.system() == "Linux":
            os.sched_setaffinity(0, [cpu_id])
        else:
            logger.warning(f"CPU affinity not supported on {platform.system()}")
...
class FractionalizedCPU:
    ...
    def worker(self, func, index, cpu_id, memory_limit, *args, **kwargs):
        try:
            # Set CPU affinity
        ...
        **kwargs: Any
        ...
        try:
            ...
            if cpu_id < 0 or cpu_id >= available_cpus:
                raise ValueError(f"Invalid CPU core: {cpu_id}. Available CPUs are 0 to {available_cpus - 1}.")
            
            if memory_fraction <= 0.0 or memory_fraction > 1.0:
                raise ValueError("Memory fraction must be between 0.0 and 1.0.")

            total_memory = psutil.virtual_memory().total
            memory_limit = int(total_memory * memory_fraction / available_cpus)

            if isinstance(func, list):
                self.result_array = Array(c_double, len(func))
                processes = []
                for i, f in enumerate(func):
                    p = Process(target=self.worker, args=(f, i, cpu_id, memory_limit) + args, kwargs=kwargs)
                    processes.append(p)
                    p.start()
                ...
                p = Process(target=self.worker, args=(func, 0, cpu_id, memory_limit) + args, kwargs=kwargs)
                p.start()
                p.join()
            ...
            logger.error(f"Error executing on fractionalized CPU {cpu_id}: {e}")
            raise
